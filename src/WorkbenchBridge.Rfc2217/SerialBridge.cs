...
/// --hexdump additionally logs hex bytes of TX data.
/// --raw logs the ASCII of BOTH directions on the cable (serial-monitor view).
///
/// The setters are intentionally mutable: the service hands a single instance to
/// the running <see cref="SerialBridge"/> and keeps a reference to it, so a live
/// `set`/`debug` toggle takes effect immediately without restarting the bridge.
/// The data/signal loops read these flags fresh on each iteration. bool field
/// reads/writes are atomic in .NET, which is sufficient for diagnostic toggles.
/// </summary>
public sealed class BridgeOptions
{
    ...
    /// <summary>
    /// Enable hex dump logging of TX data packets (IDE -> ESP32). Produces a lot
    /// of output during flash operations.
    /// </summary>
    public bool HexDump { get; set; }

    /// <summary>
    /// Enable raw ASCII logging of BOTH directions of serial traffic, rendered
    /// as printable text with control characters escaped (e.g. \r \n). This is
    /// the "what's on the cable" serial-monitor view. Like HexDump it is volume-
    /// heavy; intended for short live debug sessions, not always-on.
    /// </summary>
    public bool Raw { get; set; }

    /// <summary>
    /// Portal reset method for the slot this bridge serves: "classic", "usb-jtag",
    /// or "gpio" (null/empty = unknown, treated as classic). Read fresh by the
    /// signal loop each tick. When "usb-jtag", the bridge intercepts the host's
    /// classic DTR/RTS auto-reset and re-issues the native USB-Serial-JTAG reset
    /// sequence instead — IDEs drive only the classic sequence and offer no
    /// override, so without this a native-USB board gets the wrong reset.
    /// </summary>
...
public sealed class SerialBridge : IAsyncDisposable
{
    ...
    // Classic-reset -> USB-Serial-JTAG-reset translator, engaged only when the
    // slot's reset method (BridgeOptions.ResetMethod) is "usb-jtag". Pure state
    // machine; see Esp32ResetInterceptor.cs.
    private readonly Esp32ResetInterceptor _resetInterceptor = new();

    // Per-direction line accumulators for the raw serial-monitor view. Each is
    // only ever touched by one thread (RX by the rfc2217 receive callback, TX by
    // the local read loop), so they need no locking.
    private readonly System.Text.StringBuilder _rawRx = new();
    private readonly System.Text.StringBuilder _rawTx = new();
    ...
    /// <summary>Total bytes forwarded from the IDE to the ESP32 since start.</summary>
    public long BytesToDevice => Interlocked.Read(ref _bytesToDevice);

    /// <summary>Total bytes forwarded from the ESP32 to the IDE since start.</summary>
    public long BytesFromDevice => Interlocked.Read(ref _bytesFromDevice);

    /// <summary>Zero the TX/RX byte counters. Diagnostics only — does not touch
    /// the data path, the connection, or any logs.</summary>
    public void ResetByteCounters()
    {
        Interlocked.Exchange(ref _bytesToDevice, 0);
        Interlocked.Exchange(ref _bytesFromDevice, 0);
    }

    /// <summary>
    /// The baud rate currently programmed on the Pi side, as tracked by the
    /// SLIP sniffer / signal logic. com0com does not propagate the IDE's baud
    /// rate, so this is the authoritative "live" rate, not _localPort.BaudRate.
    /// </summary>
    public int CurrentBaudRate => _lastBaudRate;
    ...
    private void WireRfc2217Callbacks()
    {
        ...
        _rfc2217.OnDataReceived(async (data, token) =>
        {
            ...
            // Raw serial-monitor view (ESP32 -> IDE), buffered into whole lines.
            if (_options.Raw && data.Length > 0)
                AppendRaw("RX", _rawRx, data.Span);
            ...
            for (int attempt = 1; attempt <= MaxReconnectAttempts; attempt++)
            {
                ...
                try
                {
                    ...
                    if (_localPort?.IsOpen == true)
                    {
                        ...
                        // Drop any half-observed reset burst; the line baseline
                        // across a reconnect is no longer trustworthy.
                        _resetInterceptor.Reset();
                    }
        ...
        try
        {
            while (!ct.IsCancellationRequested && _localPort?.IsOpen == true)
            {
                ...
                if (bytesRead > 0 && _rfc2217 is not null)
                {
                    ...
                    // Raw serial-monitor view (IDE -> ESP32), buffered into lines.
                    if (_options.Raw)
                        AppendRaw("TX", _rawTx, buffer.AsSpan(0, bytesRead));
    ...
    /// </summary>
    /// <summary>
    /// Drive esptool's <c>USBJTAGSerialReset</c> sequence (reset.py) to the Pi with
    /// local timing, putting a native USB-Serial-JTAG chip into the download
    /// bootloader. DTR→IO0, RTS→EN; the Pi's plain RFC2217 server passes these
    /// straight to the chip. Running the sleeps locally (not relaying the host's
    /// edges over the network) keeps the sequence timing intact.
    /// </summary>
    private async Task SynthesizeUsbJtagDownloadResetAsync(CancellationToken ct)
    {
        if (_rfc2217 is null) return;
        _logger.LogInformation(
            "{Port}: classic download reset intercepted - synthesising USB-Serial-JTAG reset",
            _localPortName);

        await _rfc2217.SetRtsNoWaitAsync(false, ct);
        await _rfc2217.SetDtrNoWaitAsync(false, ct);   // Idle
        await Task.Delay(100, ct);
        await _rfc2217.SetDtrNoWaitAsync(true, ct);    // Set IO0 (BOOT low)
        await _rfc2217.SetRtsNoWaitAsync(false, ct);
        await Task.Delay(100, ct);
        await _rfc2217.SetRtsNoWaitAsync(true, ct);    // Reset (EN low)
        await _rfc2217.SetDtrNoWaitAsync(false, ct);
        await _rfc2217.SetRtsNoWaitAsync(true, ct);
        await Task.Delay(100, ct);
        await _rfc2217.SetDtrNoWaitAsync(false, ct);
        await _rfc2217.SetRtsNoWaitAsync(false, ct);   // Out of reset -> download mode
        await ResetPiBaudTo115200Async(ct);
        _lastRemoteDtr = false;
        _lastRemoteRts = false;
    }

    /// <summary>
    /// Drive esptool's <c>HardReset(uses_usb=True)</c> sequence: pulse EN to reboot
    /// the chip into its firmware WITHOUT pulling BOOT low, so it runs the app
    /// instead of re-entering the bootloader. This is what the end-of-flash
    /// <c>--after hard-reset</c> needs on a native-USB board.
    /// </summary>
    private async Task SynthesizeUsbHardResetAsync(CancellationToken ct)
    {
        if (_rfc2217 is null) return;
        _logger.LogInformation(
            "{Port}: hard reset intercepted - synthesising USB hard reset (run firmware)",
            _localPortName);

        await _rfc2217.SetRtsNoWaitAsync(true, ct);    // EN -> LOW
        await Task.Delay(200, ct);                      // USB chips need longer to settle
        await _rfc2217.SetRtsNoWaitAsync(false, ct);   // EN -> HIGH, boot firmware
    ...
    /// <summary>
    /// After a synthesised reset the chip is back at the bootloader/ROM default
    /// (115200). If the Pi is still at a previous high rate, drop it so the next
    /// esptool sync handshakes cleanly. Mirrors the classic hard-reset path.
    /// </summary>
    private async Task ResetPiBaudTo115200Async(CancellationToken ct)
    {
        if (_rfc2217 is null || _lastBaudRate == 115200) return;
        _logger.LogInformation(
            "{Port}: resetting Pi baud {Baud} -> 115200 after synthesised reset",
            _localPortName, _lastBaudRate);
        ...
        try
        {
            while (!ct.IsCancellationRequested && _localPort?.IsOpen == true && _rfc2217 is not null)
            {
                ...
                try
                {
                    ...
                    // USB-Serial-JTAG reset translation. IDEs only ever drive the
                    // CLASSIC DTR/RTS auto-reset and expose no override, so for a
                    // native-USB slot (portal reset_method "usb-jtag") we intercept
                    // the line activity and re-issue the correct USB sequence rather
                    // than mirroring the wrong one. Classic/unknown slots skip this
                    // entirely and fall through to the verbatim passthrough below.
                    if (string.Equals(_options.ResetMethod, "usb-jtag",
                                      StringComparison.OrdinalIgnoreCase))
                    {
                        // On a usb-jtag slot the interceptor is the ONLY thing
                        // that drives the device's reset lines — we never
                        // raw-forward DTR/RTS. A serial monitor holding both
                        // lines asserted is therefore a no-op (it cannot drop
                        // the chip into download mode); only esptool's real
                        // reset pulse synthesises a USB reset. Keep the
                        // passthrough's tracked state in sync so a later switch
                        // of slot/reset-method does not replay a stale edge.
                        var action = _resetInterceptor.Observe(
                            remoteDtr, remoteRts, Environment.TickCount64);
                        _lastRemoteDtr = remoteDtr;
                        _lastRemoteRts = remoteRts;

                        if (action == ResetInterceptAction.SynthesizeDownloadReset)
                            await SynthesizeUsbJtagDownloadResetAsync(ct);
                        else if (action == ResetInterceptAction.SynthesizeHardReset)
                            await SynthesizeUsbHardResetAsync(ct);
                        // PassThrough / Suppress: drive nothing on a usb-jtag slot.
                    }
                    else
                    {
    ...
    // Cap a newline-less stream so a sender that never emits \n (e.g. a binary
    // flash blob) still flushes instead of buffering without bound.
    private const int RawLineFlushAt = 512;

    /// <summary>
    /// Accumulate raw serial bytes into whole lines for the monitor view. Bytes
    /// are appended (printable kept, TAB -> \t, other non-printables -> \xNN) until
    /// a newline, at which point the line is emitted as one log entry tagged with
    /// the slot's internal port (so `logs -c`/`debug` can filter to this slot).
    /// CR is treated as part of the LF delimiter and dropped. This turns the TCP
    /// chunking ("RAW RX 1: T", "RAW RX 3: CK:") into one readable line per line.
    /// </summary>
    private void AppendRaw(string dir, System.Text.StringBuilder acc, ReadOnlySpan<byte> data)
    {
        foreach (byte b in data)
        {
            ...
            if (b == (byte)'\r')
                continue; // part of the newline delimiter
            if (b == (byte)'\t')
                acc.Append("\\t");
            else if (b >= 0x20 && b <= 0x7E)
                acc.Append((char)b);
            else
                acc.Append("\\x").Append(b.ToString("X2"));

            if (acc.Length >= RawLineFlushAt)
                FlushRaw(dir, acc);
        }
    }

    private void FlushRaw(string dir, System.Text.StringBuilder acc)
    {
        if (acc.Length == 0) return;
        _logger.LogDebug("{Port} RAW {Dir}: {Line}", _localPortName, dir, acc.ToString());
        acc.Clear();
    }
}
