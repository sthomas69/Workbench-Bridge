...
public sealed class SlipBaudRateSniffer
{
    ...
    private int? CompletePartialFrame(byte[] buffer, int length)
    {
        ...
        var parse = TryExtractBaudRate(merged, 0, combined, out int newBaud);

        if (parse == BaudParse.Valid)
        {
        ...
        // parse == Invalid: the buffered candidate was a false-positive signature,
        // not a real CHANGE_BAUDRATE. Discard it (already cleared above) and rescan
        // the merged bytes in case a genuine frame followed the false match.
        if (_verbose)
            _logger?.LogDebug(
                "SLIP sniffer: buffered candidate was a false positive, discarding and rescanning {Len} bytes",
                combined);
        return ScanForChangeBaudRate(merged, combined);
    }
    ...
    private int? ScanForChangeBaudRate(byte[] buffer, int length)
    {
        for (int i = 0; i < length; i++)
        {
                ...
                sizeLo == 0x08 && sizeHi == 0x00)
            {
                ...
                var parse = TryExtractBaudRate(buffer, bodyStart, length, out int newBaud);
                ...
                if (parse == BaudParse.Incomplete)
                {
                    // Genuinely split across reads. Buffer for next read.
                    if (_verbose)
                        _logger?.LogDebug(
                ...
                // parse == Invalid: the signature occurred by chance in the data
                // stream (common inside a compressed firmware write). NOT a real
                // command — do not buffer; keep scanning the rest of the chunk so
                // a genuine frame later on is still found.
                continue;
            }
        }
    ...
    /// <summary>
    /// Outcome of trying to read a baud rate out of a matched CHANGE_BAUDRATE
    /// signature. The caller must treat <see cref="Incomplete"/> (buffer and
    /// wait for more bytes) very differently from <see cref="Invalid"/> (the
    /// signature occurred by chance in the data — skip it and keep scanning).
    /// </summary>
    private enum BaudParse { Valid, Incomplete, Invalid }
    ...
    /// </summary>
    private BaudParse TryExtractBaudRate(byte[] buffer, int bodyStart, int bufferLength, out int newBaud)
    {
        newBaud = 0;
        ...
        if (destPos < 4)
            // Ran off the end of the buffer mid-value → genuinely split across
            // reads. Hit a frame-end C0 before 4 bytes → the body ended early,
            // so this was never a real CHANGE_BAUDRATE (a false-positive match).
            return srcPos >= bufferLength ? BaudParse.Incomplete : BaudParse.Invalid;
        ...
        {
            // The 5-byte signature (C0 00 0F 08 00) matched but the "baud" is
            // implausible — almost always the signature occurring by chance in a
            // compressed firmware stream. Expected and harmless, so debug-level
            // (not warning): at high upload speeds this fires thousands of times.
            if (_verbose)
                _logger?.LogDebug(
                    "SLIP sniffer: ignoring false-positive CHANGE_BAUDRATE signature (implausible baud {Baud})",
                    newBaud);
            newBaud = 0;
            return BaudParse.Invalid;
        }

        return BaudParse.Valid;
    }
}
