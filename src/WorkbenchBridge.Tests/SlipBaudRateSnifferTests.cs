...
public class SlipBaudRateSnifferTests
{
    ...
    public void InvalidBaudRate_TooHigh_ReturnsNull()
    {
        ...
        Assert.Null(sniffer.ProcessChunk(frame, frame.Length));
        // An implausible baud is a false positive, NOT a partial frame: the
        // sniffer must not latch into buffering on it.
        Assert.False(sniffer.HasPartialFrame);
        Assert.False(sniffer.HasDeferredBaudChange);
    }

    // ---------------------------------------------------------------
    // False-positive signature inside the data stream
    //
    // The 5-byte signature (C0 00 0F 08 00) occurs by chance inside compressed
    // firmware during a high-speed write. Regression for the bug where such a
    // match (with an implausible baud) wrongly started partial-frame buffering
    // and spammed thousands of "invalid baud" warnings.
    // ---------------------------------------------------------------

    [Fact]
    public void FalsePositiveSignature_CompleteButImplausible_DoesNotBuffer()
    {
        var sniffer = new SlipBaudRateSniffer();

        // Signature + checksum + new_baud = 0x60000000 (the real garbage value
        // seen on the wire) + old_baud + frame end.
        byte[] falseHit = {
            0xC0, 0x00, 0x0F, 0x08, 0x00,           // signature
            0x00, 0x00, 0x00, 0x00,                 // checksum
            0x00, 0x00, 0x00, 0x60,                 // new_baud = 0x60000000 (invalid)
            0x00, 0x00, 0x00, 0x00,                 // old_baud
            0xC0                                    // frame end
        };

        Assert.Null(sniffer.ProcessChunk(falseHit, falseHit.Length));
        Assert.False(sniffer.HasPartialFrame);
        Assert.False(sniffer.HasDeferredBaudChange);

        // A genuine CHANGE_BAUDRATE right after must still be detected.
        var real = BuildChangeBaudFrame(921600);
        Assert.Equal(921600, sniffer.ProcessChunk(real, real.Length));
    }

    [Fact]
    public void FalsePositiveSignature_NoTrailingC0_DoesNotLatchBuffering()
    {
        // The exact shape that spammed warnings: signature + implausible baud +
        // arbitrary trailing data and NO frame-end C0. Must not buffer.
        var sniffer = new SlipBaudRateSniffer();
        byte[] data = {
            0xC0, 0x00, 0x0F, 0x08, 0x00,           // signature
            0x11, 0x22, 0x33, 0x44,                 // checksum
            0x00, 0x00, 0x00, 0x60,                 // new_baud = 0x60000000 (invalid)
            0x55, 0x66, 0x77, 0x88                  // trailing data, no C0
        };
        ...
        // Sniffer is not stuck — a real frame on the next chunk is still found.
        var real = BuildChangeBaudFrame(460800);
        Assert.Equal(460800, sniffer.ProcessChunk(real, real.Length));
    }
